...
public abstract class BasePinotTaskGenerator implements PinotTaskGenerator {
  private static final Logger LOGGER = LoggerFactory.getLogger(BasePinotTaskGenerator.class);

  private final ClusterInfoProvider _clusterInfoProvider;

  public BasePinotTaskGenerator(ClusterInfoProvider clusterInfoProvider) {
  ...
  public List<PinotTaskConfig> generateTasks(@Nonnull List<TableConfig> tableConfigs) {
    ...
    for (TableConfig tableConfig : tableConfigs) {
      ...
      for (OfflineSegmentZKMetadata offlineSegmentZKMetadata : _clusterInfoProvider.getOfflineSegmentsMetadata(
        ...
        if (isScheduleSegment(tableConfig, offlineSegmentZKMetadata)) {
          pinotTaskConfigs.add(new PinotTaskConfig(getTaskType(), getConfigsForTask(tableConfig, offlineSegmentZKMetadata)));
          tableNumTasks++;
        }
      }
    }
  ...
  public abstract boolean isScheduleSegment(TableConfig tableConfig, OfflineSegmentZKMetadata offlineSegmentZKMetadata);
}
